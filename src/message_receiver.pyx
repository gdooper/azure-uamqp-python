...
cpdef create_message_receiver(link.cLink link_c, callback_context):
    ...
    receiver.create(<c_link.LINK_HANDLE>link_c._c_value, on_message_receiver_state_changed, <void*>callback_context)
    return receiver
...
cdef class cMessageReceiver(base.StructBase):
    ...
    cpdef open(self, callback_context):
        if c_message_receiver.messagereceiver_open(
            <c_message_receiver.MESSAGE_RECEIVER_HANDLE>self._c_value,
            <c_message_receiver.ON_MESSAGE_RECEIVED>on_message_received,
            <void*>callback_context) != 0:
                self._value_error()

    cpdef close(self):
        if c_message_receiver.messagereceiver_close(<c_message_receiver.MESSAGE_RECEIVER_HANDLE>self._c_value) != 0:
            self._value_error()

    cpdef destroy(self):
        if <void*>self._c_value is not NULL:
            _logger.debug("Destroying {}".format(self.__class__.__name__))
            c_message_receiver.messagereceiver_destroy(<c_message_receiver.MESSAGE_RECEIVER_HANDLE>self._c_value)
    ...
    cpdef set_trace(self, bint value):
        c_message_receiver.messagereceiver_set_trace(<c_message_receiver.MESSAGE_RECEIVER_HANDLE>self._c_value, value)
...
cdef c_amqpvalue.AMQP_VALUE on_message_received(void* context, c_message.MESSAGE_HANDLE recv_message):
    if context == NULL:
        return c_message.messaging_delivery_accepted()

    context_obj = <object>context
    cdef c_message.MESSAGE_HANDLE cloned
    cloned = c_message.message_clone(recv_message)
    ...
    try:
        ...
        if hasattr(e, 'rejection_description'):
            ...
            if e.annotations is not None:
                ...
                _ann = annotations.create_fields(<amqpvalue.AMQPValue>e.annotations)
